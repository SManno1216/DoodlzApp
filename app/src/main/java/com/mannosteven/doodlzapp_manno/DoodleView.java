...
public class DoodleView extends View {
    ...
    public DoodleView(Context context, AttributeSet attrs) {
        ...
        paintScreen = new Paint(); //used to display bitmap into screen

        //set the initial display settings for the painted line
        paintLine = new Paint();
        paintLine.setAntiAlias(true); //smooth edges of drawn lines
        paintLine.setColor(Color.BLACK); //default color is black
        paintLine.setStyle(Paint.Style.STROKE); //solid line
        paintLine.setStrokeWidth(5); //set default line width
        paintLine.setStrokeCap(Paint.Cap.ROUND); //rounded the line
    }
    ...
    private void touchStarted(float x, float y, int lineID){
        ...
        if(pathMap.containsKey(lineID)){
            ...
            path = new Path();
            pathMap.put(lineID, path); //add path to map
    ...
    public void saveImage(){
        //use "Doodlz" followed by the current time as the image name
        ...
        if(location != null){
            ...
            //display the message indicating that there was an error saving
            Toast message = Toast.makeText(getContext(),
